...
新增能力：
- 在正式WSD训练/验证/测试时，加载“预训练 hypernetwork 生成并保存的 dynasty adapters”
...
class WSDConfig:
    ...
    # 新增：dynasty adapter 文件路径（由 pretrain_hyper_adapter.py 生成）
    dynasty_adapter_path: Optional[str] = None

    # 新增：adapter 融合强度（可调；也可以做成可学习参数）
...
class MultiHeadWSDModel(nn.Module):
    """多分类头WSD模型 - 每个word_id一个独立分类头（新增 dynasty adapter 辅助）"""
    ...
    def train(self, train_loader, val_loader, save_dir: str = "models"):
        ...
        for epoch in range(self.config.num_epochs):
            ...
            for batch in progress_bar:
                ...
                logits_list = model(input_ids, attention_mask, word_ids, num_options, dynasties)
...
class WSDPredictor:
    """WSD预测器（测试时同样使用 dynasty adapter 辅助）"""
    ...
    def __init__(self, model_path: str, config: WSDConfig):
        ...
        # 注意：这里用当前 config（包含 dynasty_adapter_path），而不是 checkpoint['config']
    ...
    def predict(self, data: List[Dict], output_path: str):
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, desc="Predicting"):
                ...
                logits_list = self.model(input_ids, attention_mask, word_ids, num_options, dynasties)
...
def main():
    ...
    PREDICTIONS_PATH = "/root/autodl-tmp/atd/data/结果/PLM/0324-bert-pred.json"
    REPORT_PATH = "/root/autodl-tmp/atd/data/结果/PLM/0324-bert-eval.xlsx"
